...
class Attribute():
    ...
    def analyze_state(self, state, baselines):
        if isinstance(state, np.ndarray):
            state = torch.from_numpy(state/255.0).float().to(self.device)
    ...
    #     segments, bbox = self.segmentor.segment(state_seg, plot=plot)
    #     ave_att = []
    ...
    #     keep_mask = segments[ave_att > self.threshold]
    #     # bbox in XYWH format
    #     keep_bbox = bbox[ave_att > self.threshold]
